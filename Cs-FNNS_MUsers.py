...
LpipsNet = PerceptualSimilarity.models.PerceptualLoss(model='net-lin', net='alex', use_gpu=True,
                                                      gpu_ids=[0])  # For calculating LPIPS
...
for i in range(len(secret_image_path_list)):
    ...
    model_list = []
    for j in range(c.num_of_receivers):
        model = decodingNetwork(input_channel=3 * c.psf * c.psf, output_channels=3 * c.psf * c.psf,
                                down_ratio_l2=down_ratio_l2, down_ratio_l3=down_ratio_l3).to(device)
    ...
    logger.info('stego_lpips: {:.4f}, secret_rev_lpips: {:.4f}'.format(stego_lpips.mean().item(),
                                                                       secret_rev_lpips.mean().item()))
    ...
    if c.save_images == True:
        ...
        secret_rev_save_path_list = [];
        secret_resi_save_path_list = []
        for j in range(c.num_of_receivers):
            secret_rev_save_path_list.append(os.path.join(image_save_dirs, 'secret_rev', str(i) + '-' + str(j) + '-' +
                                                          secret_image_path_list[i].split('/')[-1]))
            secret_resi_save_path_list.append(os.path.join(image_save_dirs, 'secret_resi', str(i) + '-' + str(j) + '-' +
                                                           secret_image_path_list[i].split('/')[-1]))
        ...
        # 示例路径列表
        paths = [secret_rev_save_path_list, secret_resi_save_path_list]
...
logger.info('stego_psnr_mean: {:.2f}, stego_ssim_mean: {:.4f}, stego_lpips_mean: {:.4f}, stego_apd_mean: {:.2f}'.format(
    np.array(stego_psnr_list).mean(), np.array(stego_ssim_list).mean(), np.array(stego_lpips_list).mean(),
    np.array(stego_apd_list).mean()))
logger.info(
    'secret_rev_psnr_mean: {:.2f}, secret_rev_ssim_mean: {:.4f}, secret_rev_lpips_mean: {:.4f}, secret_rev_apd_mean: {:.2f}'.format(
        np.array(secret_rev_psnr_list).mean(), np.array(secret_rev_ssim_list).mean(),
        np.array(secret_rev_lpips_list).mean(), np.array(secret_rev_apd_list).mean()))
logger.info('YeNet_cover_det_acc: {:.2f}, YeNet_stego_det_acc: {:.2f}, YeNet_total_det_acc: {:.2f}'.format(
    YeNet_dec_acc[0] / num_of_imgs, YeNet_dec_acc[1] / num_of_imgs,
    (YeNet_dec_acc[0] + YeNet_dec_acc[1]) / 2 / num_of_imgs))
...
logger.info(
    'SiaStegNet_cover_det_acc: {:.2f}, SiaStegNet_stego_det_acc: {:.2f}, SiaStegNet_total_det_acc: {:.2f}'.format(
        SiaStegNet_dec_acc[0] / num_of_imgs, SiaStegNet_dec_acc[1] / num_of_imgs,
        (SiaStegNet_dec_acc[0] + SiaStegNet_dec_acc[1]) / 2 / num_of_imgs))